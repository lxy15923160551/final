...
class SDTAEncoder(nn.Module):
    def __init__(self, dim, drop_path=0., layer_scale_init_value=1e-6, expan_ratio=4,
        ...
        self.pos_embd = None
        if use_pos_emb:
            self.pos_embd = PositionalEncodingFourier(dim=dim // 4)
        self.norm_xca = LayerNorm(dim // 4, eps=1e-6)
        self.gamma_xca = nn.Parameter(layer_scale_init_value * torch.ones(dim // 4),
                                      requires_grad=True) if layer_scale_init_value > 0 else None
        self.xca = XCA(dim // 4, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop)
        ...
        self.conv2 = nn.Conv2d(dim//4, dim//4, 3, 1, 1)
        self.conv3 = nn.Conv2d(dim//4, dim//4, 3, 1, 1)
        self.conv4 = nn.Conv2d(dim//4, dim//4, 3, 1, 1)

        self.fuse =  nn.Conv2d(dim , dim, 3, 1, 1)

    def forward(self, x):
        input = x
        #print(x.shape,self.width)
        ...
        if self.pos_embd:
            pos_encoding = self.pos_embd(B, H, W).reshape(B, -1, x1.shape[1]).permute(0, 2, 1)
            
            x1 = x1 + pos_encoding
        x1 = x1 + self.drop_path(self.gamma_xca * self.xca(self.norm_xca(x1)))
        ...
        if self.pos_embd:
            pos_encoding = self.pos_embd(B, H, W).reshape(B, -1, x2.shape[1]).permute(0, 2, 1)
            x2 = x2 + pos_encoding
        x2 = x2 + self.drop_path(self.gamma_xca * self.xca(self.norm_xca(x2)))
        ...
        B, C, H, W = x3.shape
        x3 = x3.reshape(B, C, H * W).permute(0, 2, 1)
        if self.pos_embd:
            pos_encoding = self.pos_embd(B, H, W).reshape(B, -1, x3.shape[1]).permute(0, 2, 1)
            x3 = x3 + pos_encoding
        x3 = x3 + self.drop_path(self.gamma_xca * self.xca(self.norm_xca(x3)))
        x3 = x3.reshape(B, C, H, W)
        ...
        B, C, H, W = x4.shape
        x4 = x4.reshape(B, C, H * W).permute(0, 2, 1)
        if self.pos_embd:
            pos_encoding = self.pos_embd(B, H, W).reshape(B, -1, x4.shape[1]).permute(0, 2, 1)
            x4 = x4 + pos_encoding
        x4 = x4 + self.drop_path(self.gamma_xca * self.xca(self.norm_xca(x4)))
